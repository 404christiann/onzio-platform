...
| **New:** are `timebox` / `inactivity_timeout` available on Supabase Free? | `PLAT-101` | Open — verification | Staging is Free while production is Pro. If these are paid-plan features, `PLAT-D016` cannot be rehearsed on staging and `PLAT-101`'s acceptance criterion that "club sessions persist across the configured window" needs restating. `PLAT-D015` is unaffected, being application code |
| **New:** does Vercel notify on a failed cron for this account? | `PLAT-102` | Open — verification | `PLAT-D019` escalates by returning non-200. If Vercel only records the failure rather than notifying, the alert is a pull and not a push, and the delivery half of the reconciliation question is not truly closed |
