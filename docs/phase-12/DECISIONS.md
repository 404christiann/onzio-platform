...
| PLAT-D024 | accepted | **Club admins keep full content editing during the grace window.** `can_mutate_content` admits `public_access in ('live', 'grace')` for `customer` clubs, not `'live'` alone. Suspension at day 20 remains the only enforcement step. | `PLAT-102` shipped a read-only-admin-during-grace behaviour that no decision authorised — it emerged from `can_mutate_content` requiring `public_access = 'live'`. Reviewed and rejected on 2026-08-03 for three reasons. (1) The public site stays **up** during grace, so freezing edits leaves a live site showing information the club is powerless to correct — a cancelled match or a stale tryout date — which is worse for their public than a clean suspension. (2) The person who edits content is an `admin`; the person whose card expired is the `owner`. Freezing the admin does not accelerate payment. (3) Decisively, **`PLAT-D006`'s kill switch does not reach this behaviour**: `LIFECYCLE_SUSPENSION_ENABLED` only stops the cron's grace→suspended write, while the edit lock is driven by `public_access = 'grace'`, which the **webhook** sets. In the exact fault `PLAT-D006` accepts as a risk — a webhook or cron fault wrongly marking a paying customer unpaid — the documented escape hatch would keep their site up but still leave them unable to edit. That is an unmitigated second automated path from an infrastructure fault to customer harm, which is precisely what `PLAT-D006`'s mitigations exist to bound. Grace means warned and fully working; day 20 is the lever. Requires a test pinning the behaviour so it cannot regress silently. |
...
| --- | --- | --- | --- |
| Automated suspension can take a paying customer's public site offline on a webhook or cron fault | `PLAT-D006` | **Accepted 2026-08-03** with the decision | Measured from `paid_through`; 20-day buffer; mandatory warning notices; `actor_type = 'system'` audit rows; cron disableable by configuration without a deploy. **Scope note added 2026-08-03:** `LIFECYCLE_SUSPENSION_ENABLED` gates only the cron's grace→suspended write. It does not reach behaviour keyed off `public_access = 'grace'`, which the webhook sets. `PLAT-D024` removed the one such behaviour that existed; any future grace-keyed restriction must either be covered by the switch or accepted as a separate risk |
