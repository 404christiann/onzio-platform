...
| Email deliverability becomes a hard authentication dependency | `PLAT-101` | **Accepted 2026-08-03** with the exact staging package approval | Dedicated transactional subdomain with SPF/DKIM/DMARC; bounce and failure monitoring; delivery tested against Yahoo, AOL, and at least one ISP-hosted domain, not only Gmail; operator email-change escape hatch |
