...
**Promotion records a decision under governance; it does not accept it.** No
entry below carries Christian's dated approval yet, so every one is
`promoted_awaiting_acceptance`. `PLAT-101` names `PLAT-D012`, `PLAT-D013`, and
`PLAT-D014` as accepted dependencies, and `PLAT-102` names `PLAT-D003`,
`PLAT-D004`, and `PLAT-D006`–`PLAT-D011`; neither package may be assigned until
those entries move to `accepted` with a date and approver in the Acceptance
Record below.

Two decisions carry an explicit accepted risk rather than a neutral trade-off:
`PLAT-D006` (automated suspension) and `PLAT-D012` (single-factor club
accounts). Accepting either accepts its risk. Both are restated in the Accepted
Risk Register.
...
| --- | --- | --- | --- |
| Automated suspension can take a paying customer's public site offline on a webhook or cron fault | `PLAT-D006` | Explicitly accepted in the decision | Measured from `paid_through`; 20-day buffer; mandatory warning notices; `actor_type = 'system'` audit rows; cron disableable by configuration without a deploy |
| Inbox access equals club content access | `PLAT-D012` | Explicitly accepted in the decision | All privileged actions confined to operator accounts under mandatory TOTP (`PLAT-D013`) |
| Email deliverability becomes a hard authentication dependency | `PLAT-101` | Accepted with the package | Dedicated transactional subdomain with SPF/DKIM/DMARC; bounce and failure monitoring; delivery tested against Yahoo, AOL, and at least one ISP-hosted domain, not only Gmail; operator email-change escape hatch |
| Card-only excludes clubs requiring purchase orders | `PLAT-D011` | Accepted for now | Revisit together with the `PLAT-D006` timer assumption when an academy requires it |
...
| Operator power is an environment-variable allowlist requiring a deploy to revoke | `PLAT-D013` | Accepted at one operator | Move to an audited `operators` table at the second operator |
...
| Item | Owning package | Note |
| --- | --- | --- |
| Grace warning schedule | `PLAT-102` | Proposed day 7 and day 17 of the 20-day window |
| Final form of `club_has_feature` | `PLAT-102` | A lifecycle-and-kind check, or removal with callers repointed at `can_read_club` / `can_mutate_content` |
| Reconciliation report cadence and delivery | `PLAT-102` | Compares `clubs.stripe_price_id` (intent) with `club_subscriptions.price_id` (fact); divergence is a finding, never a webhook rejection |
| Whether AAL2 survives session refresh | `PLAT-101` | Determines whether operator re-authentication is per-session or per-refresh |
| Exact session durations for club and operator accounts | `PLAT-101` | Asymmetric by design: weeks for club accounts, hours for operator accounts |

## Acceptance Record

No decision below has been accepted yet. Record the date, approver, and
evidence here when acceptance is given, and update the status column in
Promoted Decisions to match.
