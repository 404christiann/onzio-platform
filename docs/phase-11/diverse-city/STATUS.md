...
## 2026-08-10 - Stale deployment alias, admin restyle test fixes, and DCFC-901 checkout price gap — Claude Code (Sonnet 5)

- Package: not a scoped work-package assignment; ad hoc production troubleshooting across the day, ending on `DCFC-901`'s checkout blocker.
- Status: `diverse-city-fc-private.vercel.app` public routes and `/admin` are confirmed live and correct. `DCFC-901`'s checkout blocker is resolved. `DCFC-701`'s Rose City billing-drift blocker is unchanged (not touched today).
- Completed work:
  - Root-caused an apparent middleware tenant-resolution bug (the subject of four "temporary diagnostic header" commits, `e7adc70`..`6e1aca2`) to a **stale Vercel alias**: `diverse-city-fc-private.vercel.app` was manually pinned via `vercel alias set` to a deployment 3 commits behind branch HEAD, and was never a registered Vercel project Domain, so it never auto-followed new deployments. No middleware code defect existed.
  - Reverted the four diagnostic-header commits (`7fae4d0`, merged to `staging`).
  - Discovered this project's Vercel Production Branch is `main`, not `staging`; plain pushes to `staging` always build Preview (different env vars than Production). Left this as-is per Christian's explicit choice to preserve the manual `--prod` gate rather than risk unattended production deploys on every staging push.
  - Registered `diverse-city-fc-private.vercel.app` as a proper Vercel project Domain (`vercel domains add`) so it now auto-follows the latest Production-target deployment via the project's existing `autoAssignCustomDomains`.
  - Reviewed the admin-portal restyle scope creep (`534cb9b`, `3f4d5fe`, done by a different session continuing from a shared styling task) against the full contract suite: 3 failures traced to legitimate componentization (Sidebar, `FileUpload`, `PaymentStatusCard` extracted from inline markup) that left source-text-matching tests checking stale locations — no actual functionality or compliance copy was dropped. Fixed the 3 tests, and restored 5 viewport-clamp Tailwind classes the Sidebar extraction had genuinely dropped versus the original `<aside>` (`max-h-screen max-h-[100dvh]`, `lg:flex`, `lg:h-screen lg:h-[100dvh]`, `lg:max-h-screen lg:max-h-[100dvh]`, `lg:self-start`). Committed as `3dacd50`, pushed to `origin/staging`.
  - Found `/api/stripe/checkout` returning `{"error":"STRIPE_PRICE_REQUIRED"}` on the live admin Payments tab: `clubs.stripe_price_id` for `diverse-city` (`d7a41762-5158-496e-b415-c83c01ab5c70`) had never been set. `DCFC-D126` recorded a $75/month Price as the intended value but it was never written to the row; by the time this was fixed, Christian had separately negotiated $85/month with the client on a new dedicated Product (`prod_V34NKDxtbguIn0` / `price_1U2yF0K6WajTkwHYjgvrqAly`), a deliberate one-club exception to `DCFC-D128`. Wrote and ran `scripts/set-diverse-city-live-price.ts` (Christian ran it himself with the production service-role key; I do not have that key). Recorded as `DCFC-D129` in `DECISIONS.md`.
- Files changed: `middleware.ts` (diagnostic headers removed), `components/ui/sidebar.tsx`, `components/AdminShell.tsx`, `tests/contracts/admin-mobile-navigation.test.ts`, `tests/contracts/diverse-city-admin-punch-list.test.ts`, `tests/contracts/plat-102-billing-model.test.ts`, `scripts/set-diverse-city-live-price.ts` (new), `docs/phase-11/diverse-city/DECISIONS.md`.
- Verification: `npx tsc --noEmit` clean throughout. `npm run test:contracts` 496/496 passing (was 492/496 before the test fixes). `npm run test:legacy` 274/274 passing. Live-verified `diverse-city-fc-private.vercel.app` root, `/admin` login-through-dashboard, and the Payments tab checkout dry run / write / reconciliation output, all via the browser and via Christian directly running the price-fix script and pasting output back.
- Blockers or unresolved decisions: `DCFC-701` (Rose City Stripe/DB billing-projection drift) is still open and untouched — separate from today's work. The stale top-level `ROLLOUT-EPIC.md`/`ROLLOUT-WORK-PACKAGES.md` planning docs (last updated 2026-08-01) were noted as ~5 days behind this file and `HANDOFF.md`; not reconciled today.
- Exact next step: verify the Payments tab checkout flow end-to-end now that `clubs.stripe_price_id` is set (Stripe Checkout session should create successfully; confirm the $85 price and Diverse City FC branding appear correctly on the Checkout page itself). Once confirmed, `DCFC-602`'s public/admin acceptance sweep is next, still gated on `lifecycle=onboarding` forcing `public_access=preview` until `DCFC-901`'s actual owner Checkout completes.

## 2026-08-11 - Cross-tenant "Rose City FC" hero first-paint flash fixed platform-wide — Claude Code (Fable 5)

- Package: not a scoped work-package assignment; production defect reported by Diverse City FC (customer saw "ROSE CITY FC" / "TEAM STORE" / "MEET THE SQUAD" flash on their homepage before their own content painted, desktop and mobile).
- Status: complete locally on `staging`; not committed, not pushed, not deployed.
- Root cause: `components/Hero.tsx` initialized `useState(DEFAULT_HOMEPAGE_HERO_CONTENT)` — a constant carrying Rose City's real copy — then swapped in the tenant's content only after a client-side `fetchHomepageContent(club.id)` resolved. Every non-Rose-City club's homepage first-painted Rose City branding; Rose City itself never showed the flash because its real content matches the default, so Diverse City (first other club fully live) was the first to see it.
- Completed work:
  - `app/%5Fclubs/[slug]/page.tsx` no longer re-exports the client homepage; it is now a server component (same pattern as `app/%5Fclubs/[slug]/club/about/page.tsx`) that resolves the hero via new `fetchHomepageHeroContent(club.id, serverClient)` before any HTML is sent and passes it down, so the first paint is already the tenant's own copy.
  - Client homepage moved to `components/HomePageClient.tsx` (AboutClubPageClient convention) with a required `initialHeroContent: DBHomepageHeroContent | null` prop, forwarded through `ClubhouseHomePage` into `Hero`, whose prop is also required — no consumer can silently fall back to another club's branding again.
  - `Hero` keeps a client-side fetch only for `initialContent === null` (legacy unscoped route or failed server fetch), now starting from new tenant-neutral `EMPTY_HOMEPAGE_HERO_CONTENT` (all-blank; templates fall through to `club.name` and generic CTA labels) instead of Rose City copy. This also closes the latent case where the fetch *errored* and the Rose City default stuck permanently.
  - `lib/queries.ts`: hero leg of `fetchHomepageContent` extracted into `fetchHomepageHeroContent(clubId?, client?)` with an injectable Supabase client (mirrors `fetchAboutClubContent`); the DCFC-602 tenant-safe fallback now uses the shared `EMPTY_HOMEPAGE_HERO_CONTENT` constant. Behavior byte-identical for every club with a saved hero row.
- Files changed: `components/Hero.tsx`, `components/HomePageClient.tsx` (new), `components/ClubhouseHomePage.tsx`, `app/%5Fclubs/[slug]/page.tsx`, `app/(public)/page.tsx`, `lib/homepage-content.ts`, `lib/queries.ts`, `tests/contracts/homepage-hero-content.test.ts`, `tests/contracts/tenant-safe-empty-content.test.ts`.
- Verification: `npx tsc --noEmit` clean; `npm run lint` clean (pre-existing admin warnings only); `npm run test:contracts` 521/521 (3 new tests), `npm run test:legacy` 274/274, `npm run test:architecture` 20/20; `npm run build` clean. Live-verified against local Supabase staging: `diverse-city` (academy@1) serves `initialHeroContent` = "One Club / One Community" in the document payload with zero "Rose City"/"Team Store" occurrences, rendered correctly in the browser; `lions` (clubhouse@1) likewise serves its own "Capital City. / Roar as One." hero. `alpha` could not be exercised locally — pre-existing data gap (no `staging`-environment `club_domains` row → `PRIMARY_DOMAIN_REQUIRED` in the tenant layout, unrelated to this change).
- Blockers or unresolved decisions: audit found two latent server-side cross-tenant fallbacks left un-fixed deliberately (flagged, not changed): `fetchAboutClubContent`'s `logo` field falls back to Rose City's `DEFAULT_CLUB_LOGO_PAGE_CONTENT` even for tenant-scoped calls (the `about` field has a tenant-safe guard; `logo` does not), and `diffHomepageSlideshowPhotos` writes `"Rose City FC homepage slide N"` alt text into any club's rows when an admin saves a photo with a blank alt.
- Exact next step: commit/push is Christian's call. Recommended follow-ups: tenant-safe empty fallback for the club-logo page content, and club-name-based alt fallback in `diffHomepageSlideshowPhotos`.

## 2026-08-11 - Both flagged follow-ups closed: tenant-safe Club Logo fallback and neutral slideshow alt text — Claude Code (Fable 5)

- Package: direct follow-up to the entry immediately above; not a scoped work-package assignment.
- Status: complete, independently re-verified, committed and pushed by the orchestrating session (see the session wrap-up entry below).
- Completed work:
  - `lib/about-content.ts`: new `EMPTY_CLUB_LOGO_PAGE_CONTENT` (mirrors `EMPTY_ABOUT_PAGE_CONTENT`'s convention — blank strings, empty arrays), matched against `DBClubLogoPageContent`'s real shape in `lib/db-types.ts`.
  - `lib/queries.ts`: `fetchAboutClubContent`'s `logo` field now branches `clubId ? EMPTY_CLUB_LOGO_PAGE_CONTENT : DEFAULT_CLUB_LOGO_PAGE_CONTENT`, exactly like the `about` field beside it. Previously any tenant-scoped call with no saved `club_logo_page_content` row got Rose City's real crest-story map image and features served server-side, permanently — not just a flash.
  - `app/%5Fclubs/[slug]/club/logo/page.tsx`: catch-path fallback changed to `EMPTY_CLUB_LOGO_PAGE_CONTENT`, matching the tenant about page's existing convention. The `(public)` legacy route keeps `DEFAULT_*`, matching the `(public)` about page — Rose City's own path unaffected.
  - `lib/homepage-content.ts`: `diffHomepageSlideshowPhotos`'s blank-alt fallback changed from `` `Rose City FC homepage slide ${index + 1}` `` to `` `Homepage slide ${index + 1}` `` — no signature change; this exact generic phrasing was already the render-time convention used elsewhere in the same admin page. Previously this wrote the Rose City string into *any* club's own `homepage_slideshow_photos.alt` column when an admin saved a photo with a cleared alt field — real data corruption risk, not just a render artifact.
  - Consumers verified safe with blank Club Logo content: `ClubLogoPageClient` already guards `annotated_image_url &&` / `map_image_url &&` and only `.map`s the feature/color-card arrays.
- Files changed: `lib/about-content.ts`, `lib/queries.ts`, `app/%5Fclubs/[slug]/club/logo/page.tsx`, `lib/homepage-content.ts`, `lib/__tests__/homepage-content.test.ts`, `tests/contracts/tenant-safe-empty-content.test.ts`.
- Verification (both by the implementing agent and independently re-run by the orchestrating session before commit): `npx tsc --noEmit` clean; `npm run lint` clean (pre-existing warnings only); `npm run test:contracts` 522/522; `npm run test:legacy` 275/275; `npm run test:architecture` 20/20; `set -a && . ./.env.test && set +a && npm run test:db` 174/174 (real local Postgres); `npm run build` clean. Live-verified locally: `/_clubs/diverse-city/club/logo` renders Diverse City's own chrome with the empty logo body, zero "Rose"/"Pasadena" strings.
- Blockers or unresolved decisions: two items explicitly flagged, not fixed —
  1. The admin **draft editor** (not the public site) for a brand-new club with no saved About/Logo content still seeds the edit form itself with Rose City's real copy (`normalizeClubLogoFeatures([])`/`normalizeStoryParagraphs([])` fall back to `DEFAULT_*`). The original About-page `EMPTY_*` fix accepted this identically as existing precedent, so scope wasn't expanded here either — same bug class, next instance if anyone wants it.
  2. Whether any real club besides Rose City already has a `homepage_slideshow_photos` row with the old branded alt text baked in from before this fix is unconfirmed — nobody has checked production data for this (uploads normally seed alt from the filename, so it only fires if an admin deliberately cleared the field first; plausible but likely rare).
- Exact next step: none required for this package. If picking up the two flagged items above, they're small, well-scoped, and independent of each other.

## 2026-08-11 - Session wrap-up: Diverse City FC publicly live — domain, real owner, real billing, confirmed working end to end

Agent: Claude Sonnet 5 (Claude Code). Status: complete, verified live in production. This is a summary entry; see `HANDOFF.md`'s matching entry for the full narrative and the two entries above (and the Stripe-portal entry in `HANDOFF.md`) for implementation detail on the pieces built by Fable subagents this session.

- Domain: `diversecityfc.com` + `www` bought through Vercel, attached to production as primary/secondary via new `lib/operator/attach-club-domain.ts` (8/8 tests against real local Postgres) and a one-time script. Old `diverse-city-fc-private.vercel.app` alias still works, demoted not removed.
- Owner: real club owner invited into production as a second `owner` via new `scripts/invite-diverse-city-owner-production.ts`. Christian decided to remain co-owner rather than the original one-owner plan — recorded as `DECISIONS.md`'s `DCFC-D133`, superseding only `DCFC-D113`'s owner-count clause.
- Billing: owner completed real Stripe Checkout. `diverse-city` is confirmed `lifecycle=active`, `public_access=live` in production. Same-day incident: `STRIPE_PORTAL_CONFIGURATION_ID` missing in Production only (Preview had it) broke "Manage billing" for the first real customer to click it; fixed live within the turn, then hardened (widened shared Stripe config validation, new `npm run stripe:verify-portal-config` live-check script, friendly billing-error UI replacing raw JSON — full detail in `HANDOFF.md`'s Stripe entry). Also found and fixed live: Stripe Portal's `subscription_cancel` was enabled, contradicting the app's own "self-serve cancellation disabled" copy — disabled directly in the Stripe Dashboard (no MCP write path existed for this), confirmed matching `lib/stripe-portal.ts` afterward.
- Also found and fixed, unrelated to the launch sequence itself: Payments page price display read a nonexistent global env var instead of the club's own `stripe_price_id` (would never have shown a price for any club); and the cross-tenant Rose City content-leak bug class covered in the two entries above.
- Current state confirmed live via curl/browser: both domains serve the real public site, no auth wall, real crest in `og:image`, admin login works, Payments shows the real $85/mo price, Portal opens with cancellation correctly disabled, homepage no longer flashes Rose City branding. Deployed from `staging` commit `6366f47`.
- Process note: `~/Downloads/onzio-platform`'s working tree was found checked out to an unrelated branch (`claude/lions-fc-website-setup-ij0p7t`) partway through this session, almost certainly a concurrent session sharing the same clone. This entry and the `HANDOFF.md` counterpart were written from a separate `git worktree` on `staging` specifically to avoid touching that. Confirm the checked-out branch before assuming it's `staging`.
- Blockers or unresolved decisions: `diverse-city-onzio-staging.vercel.app` alias likely stale (last repointed mid-session, before several later commits) — repoint when next asked to. The two flagged-not-fixed items from the entry directly above. Nothing else outstanding from this session's work.
- Exact next step: none required. This session's Diverse City work is complete.
