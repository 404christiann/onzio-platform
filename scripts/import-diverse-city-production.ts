...
const TARGET_TENANT_ID = "d7a41762-5158-496e-b415-c83c01ab5c70";
const TARGET_HOSTNAME = "diverse-city-fc-private.vercel.app";
...
function parseArgs(argv: string[]) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (["confirm-production", "prepare-sql", "sync-storage"].includes(key)) {
      result[key] = true;
      continue;
    }
...
function productionRows(rows: DiverseCityLocalImportRows) {
...
async function assertHostedTarget(service: ServiceClient) {
  ...
  if (
    ...
    throw new Error("The hosted tenant no longer matches the approved DCFC-801 target.");
  }
  ...
  if (domainCount !== 1) throw new Error("The approved production domain is not exact.");
  ...
  if (ownerCount !== 1) throw new Error("The production tenant must have exactly one active owner.");
}

function buildSql(rows: ReturnType<typeof productionRows>) {
  const statements = [
...
begin
  ...
  ) then raise exception 'DCFC-802 target tenant mismatch'; end if;
  if (select count(*) from onzio.club_domains
      where club_id = '${TARGET_TENANT_ID}'::uuid and hostname = '${TARGET_HOSTNAME}'
        and environment = 'production' and active and is_primary) <> 1
  then raise exception 'DCFC-802 target domain mismatch'; end if;
  if exists (select 1 from onzio.club_subscriptions where club_id = '${TARGET_TENANT_ID}'::uuid)
  then raise exception 'DCFC-802 cannot run after billing projection'; end if;
  ...
  if not exists (
    ...
    where club_id = '${TARGET_TENANT_ID}'::uuid
      and operation = 'diverse_city_production_import'
    ...
    values
      ('${TARGET_TENANT_ID}'::uuid, v_owner, 'migration',
       'diverse_city_production_import', 'club', '${TARGET_TENANT_ID}',
       '{"plan_digest":"${APPROVED_PLAN_DIGEST}","retained_assets":10,"destination":"production"}'::jsonb);
  end if;
end
$dcfc802$;
select jsonb_build_object(
  ...
  'import_audits', (select count(*) from onzio.audit_events where club_id = '${TARGET_TENANT_ID}'::uuid and operation = 'diverse_city_production_import')
) as dcfc_802_result;
`;
}
...
async function main() {
  ...
  if (args["confirm-production"] !== true) {
    throw new Error("DCFC-802 production execution requires --confirm-production.");
  }
  ...
  if (args["prepare-sql"] === true) {
    const sqlOut = typeof args["sql-out"] === "string"
      ...
      : resolve("/private/tmp/dcfc-802-import.sql");
    const sql = buildSql(productionRows(buildDiverseCityLocalImportRows(plan)));
