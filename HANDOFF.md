...
The read-only review found a homepage stall behind optional story data, a
program pathway stall behind optional copy, and hidden-page scroll/focus
interaction during hero media loading. Subagents fixed each issue. A second
review found two related gaps: the menu and hero scroll locks could release
each other, and footer links were focusable beneath the cover. Those are now
fixed with a shared scroll lock and academy-scoped footer visibility. The
final read-only review reported no findings.

**Files:** homepage route; Academy Programs Pathway and its data loader;
academy loading surface, Hero, HomePageClient, Nav, tenant layout, and shared
scroll lock; focused regression contracts. The selected design remains the
real header over a white centered loading state. Design-preview HTML and its
assets remain local and are excluded from the feature PR.

**Verification:** rebased onto `origin/main` at `0e76a5c`; the integrated
production build and full local-only Supabase suite 1601/1601 passed. Before
the rebase, academy loading regressions 18/18, contracts 964/964, and
architecture 21/21 also passed. Chromium at 390px held the hero video request: the white
cover stayed up, footer was hidden, lower content was inert, scrolling stayed
at the top, the menu opened, and the page remained locked after the media
fallback while the menu was still open. Closing the menu restored scrolling;
there were no page errors. The rebuilt homepage rendered in iPhone 17 Pro
Simulator Safari. Touch interaction there is still pending because the Mac
was locked during the final check. After rebasing, eleven public routes returned
...
Christian rejected the Clear Frame placeholder treatment and selected
Familiar Blend / Header Stays (option 2 in
\`diverse-city-apple-loading-options.html\`). The academy public site now
keeps its real, functional navbar above a white full-page loading surface
with a restrained activity ring and centered, route-specific message.
The direct-load fallback also renders the real navbar while branding is
pending; it uses the tenant-scoped \`/club-logo\` route until branding resolves
and falls back to club initials if the image fails. The header uses its light
appearance while the page or homepage media is loading, then returns to its
original transparent hero treatment when ready. Client route transitions
retain the settled navbar. The homepage loading surface covers the entire
viewport until the hero video or poster is visible, including on tall mobile
screens; section-level reads use the same quiet message rather than pale
placeholder shapes. The Programs dropdown likewise uses a small text
indicator instead of placeholder bars. Other presentation templates keep
their prior behavior.

**Files:** \`app/%5Fclubs/[slug]/layout.tsx\`,
...
this handoff, and the Diverse City status ledger. All work remains local and
uncommitted in \`/private/tmp/onzio-diverse-city-loading-options\` on
\`codex/diverse-city-loading-options\`.

**Verification:** focused loading contracts 14/14, TypeScript, build, and
diff check passed. The combined full suite passed 1585/1587; two unrelated
operator-workflow database checks timed out while local Supabase was slow,
then passed on a focused rerun. A separate database suite passed 245/245 and
all non-database tests passed 1342/1342. Built local browser checks with
delayed video and roster requests showed the real header and centered
loading message at 390px and 1440px, then the settled hero/roster with no
browser errors or horizontal overflow. The mobile hero loader covered the
full 390×844 viewport, and the menu opened while the video waited.
The Men’s Teams program page returned 200 at 390px without overflow.
The temporary preview server on port 3335 was stopped after verification;
no other local service was stopped, and no hosted mutation, commit, push,
or deployment occurred.

**Exact next step:** Christian visually reviews option 2 in the app,
including direct loads and client navigation on desktop/mobile, then decide
whether to commit the branch and open a PR. Keep release approval separate:
do not push or deploy without Christian's explicit authorization.

## Academy Clear Frame stall fallbacks — 2026-09-24 (Codex)

Christian requested fixes for two read-only review findings on the local
`codex/diverse-city-loading-options` branch. The academy shell no longer
waits for the hydrated Programs query before rendering every page. The Nav
loads Programs independently; its dropdown shows the Clear Frame shapes while
pending, then a usable unavailable state after five seconds if the read stalls.
A late successful response still replaces that state with real links. The
request-scoped branding read retains the first-paint crest on healthy requests
but falls back after 2.5 seconds so the outer Suspense frame cannot remain
indefinitely. When the client branding retry remains pending, the navbar
switches from its skeleton to functional links with club initials after five
seconds; a late success still supplies the crest.

**Files:** `app/%5Fclubs/[slug]/layout.tsx`,
`components/Nav.tsx`, new `lib/academy-shell-branding.ts`, new
`tests/contracts/{academy-programs-loading-stall,academy-nav-branding-timeout}.test.ts`,
this handoff, and the Diverse City status ledger. Other prior Clear Frame
changes remain uncommitted in the same isolated worktree.

**Verification:** focused tests 11/11, contracts 956/956, architecture
21/21, TypeScript, all non-database tests 1342/1342, local database
245/245, full suite 1587/1587, production build, and `git diff --check`
passed. The first database attempt could not reach `127.0.0.1:54321` from
the sandbox; the same suite and full suite passed with approved local-only
loopback access to the already-running Supabase containers. The local preview
was rebuilt and restarted at `http://diverse-city.localhost:3225/`.
Fresh desktop (1440px) and mobile (390px) pages returned 200, rendered
Programs and the real navbar with no browser errors or horizontal overflow.
A browser-intercepted stalled Programs request left the hero and navbar
usable, then changed the Programs dropdown from loading shapes to
`Programs unavailable` after the budget. The forced branding stall is
covered by fake-timer regression tests but was not forced in the browser.
No hosted mutation, commit, push, or deployment was made.

**Exact next step:** visually check the forced branding fallback and
reduced-motion/slow-network transitions on desktop and mobile, then review
the branch before release. Do not push or deploy without Christian's
explicit release approval.

## Academy hero waits for a visible video frame — 2026-09-24 (Codex)

Christian reproduced a mobile and desktop gap on the Diverse City homepage:
once hero copy data resolved, the Clear Frame skeleton disappeared before the
Bunny hero video had a frame, exposing the plain navy background with finished
text. The academy Hero now mounts the video behind its loading frame and keeps
the real copy hidden until the video emits `loadeddata`/`playing`. The video
component reports visual readiness to Hero. If autoplay fails or the media
request remains stalled for 10 seconds, it switches to its existing local
poster and reports readiness when that image loads. Other templates and the
story video keep their existing presentation.

**Files:** `components/{Hero,ResilientBunnyVideo}.tsx`,
`tests/contracts/academy-clear-frame-loading.test.ts`, this handoff, and the
Diverse City status ledger. All changes remain local on
`codex/diverse-city-loading-options` in
`/private/tmp/onzio-diverse-city-loading-options`.

**Verification:** focused 7/7, TypeScript, contracts 949/949, architecture
21/21, local database 245/245, full suite 1580/1580, production build, and
`git diff --check` passed. The rebuilt preview is running at
`http://diverse-city.localhost:3225/`. Direct desktop and 390px mobile reloads
showed the poster-backed skeleton with the hero text hidden at first paint;
the settled page had no busy skeleton, a playing video at readyState 4, visible
copy, and zero mobile horizontal overflow. The 10-second stall fallback was
covered by code review/source regression but was not forced in the browser.

**Next step:** Christian reviews the homepage reload on mobile and desktop.
Do not push or deploy without explicit release approval.

## Academy Clear Frame all-page and mobile coverage — 2026-09-24 (Codex)

Christian requested coverage across the Diverse City public site and mobile.
The shared `academy@1` loading frame now maps Home, Programs, program detail,
Tryouts, Contact, Sponsors, Store, About, Roster, Fixtures, Club Logo, and
registration to geometry based on the actual first screen of each route.
Other public URLs use a simple light frame. The new tenant-segment
`loading.tsx` supplies page-only geometry during client navigation while the
settled navbar remains in place; non-academy templates keep their existing
pending behavior. Store, Roster, and Fixtures client fetches now show matching
Clear Frame skeletons instead of text loaders. The treatment stays static and
tenant-neutral. No admin route or other template was changed.

**Files:** `app/%5Fclubs/[slug]/loading.tsx`,
`app/(public)/{roster,schedule}/page.tsx`,
`components/{AcademyLoadingSkeleton,AcademyRouteLoadingSkeleton,AcademyShopPage}.tsx`,
`lib/academy-loading-route.ts`,
`tests/contracts/academy-clear-frame-loading.test.ts`, and this handoff/status
ledger. The earlier uncommitted implementation and design previews remain in
the isolated `codex/diverse-city-loading-options` worktree.

**Verification:** focused 6/6, TypeScript, contracts 948/948, architecture
21/21, local database 245/245, full suite 1579/1579, final `next build`, and
`git diff --check` passed. Local browser
captures at 390px showed direct-load Store, About, Roster, and Club Logo frames,
plus a client navigation from Roster to Fixtures with a settled navbar and a
page-only frame. Those captures had zero horizontal overflow; About and
Roster settled with no busy skeleton remaining. After the final build, the
Store frame was checked again at 320px: its image panel matches the 420px
mobile height, horizontal overflow is zero, and the real jersey page replaces
the skeleton. The local preview is running on port 3225.

**Next step:** Christian reviews the preview on port 3225 on mobile and
desktop. Slow network and
reduced-motion acceptance remain open. Do not push or deploy without explicit
release approval.

## Academy interior Clear Frame correction — 2026-09-24 (Codex)

Christian's Men’s Teams screenshot showed the generic non-home fallback as
small pale shapes crowded against the top of a mostly empty light page. The
real program detail route opens on a full-viewport dark photo hero with its
copy near the bottom. Replaced that fallback with a full-height navy Clear
Frame hero and bottom-aligned text geometry, followed by a section frame.
The Programs index and other navy-intro routes now use a tall navy band and
content below; Store and other light routes use a full-height split frame.
The navbar skeleton uses a circular crest shape and light/dark tones to match
its page. All remain static, tenant-neutral, and limited to pending states.

**Files:** `components/{AcademyLoadingSkeleton,AcademyRouteLoadingSkeleton}.tsx`,
`lib/academy-loading-route.ts`, and
`tests/contracts/academy-clear-frame-loading.test.ts`.
**Verification:** focused Clear Frame regression 4/4, TypeScript, contracts
946/946, architecture 21/21, full suite 1577/1577, `git diff --check`, and
`next build` passed. Captured actual local loading frames at
`diverse-city.localhost:3225`: Men’s Teams desktop and 390px mobile now place
the hero shapes near the bottom; Programs index has a navy intro plus cards;
Store has a light split frame. The settled Men’s Teams route still returns
200, and mobile horizontal overflow is zero. The local server was rebuilt and
restarted on port 3225; no push or deployment was made.

**Next step:** Christian can reload the Men’s Teams route and review the
loading transition. Keep release approval separate from this local preview.

## Academy Clear Frame loading implementation — 2026-09-24 (Codex)

**Status: in progress for visual acceptance.** Christian selected Clear Frame
(option 2 of `diverse-city-poster-veil-variations.html`). The shared
`academy@1` public shell now streams a route-aware static loading frame while
tenant branding and Programs are resolved. Homepage loading shows the existing
hero poster under a 3px blur/navy veil with pale-blue, 5px-radius shapes and
no shimmer or artificial delay. Non-home academy routes use the same navbar
frame with neutral page geometry. The server layout passes request-scoped
branding and active Programs to the client; a failed branding read shows the
crest-shaped navbar skeleton during the client retry instead of flashing
initials. A settled empty/error Programs result shows text rather than a
permanent skeleton. Hero and story copy are server-resolved; failed server
reads keep their skeletons through the client retry. Academy shop, match,
slideshow, sponsors, standings, pathway, and optional video sections hold
geometry while their real client reads are pending, then render content or
their existing empty/hidden state. The match and hero CTA no longer wait behind
delayed reveal animation after the skeleton clears.

**Changed implementation:** `app/%5Fclubs/[slug]/{layout,page}.tsx`,
`components/{AcademyLoadingSkeleton,AcademyRouteLoadingSkeleton,AcademyHomeShopFeature,AcademyNextMatch,AcademyProgramsPathway,BehindTheRose,ClubBrandingProvider,DevelopingNextGeneration,Hero,HomePageClient,LeagueStandingsContainer,Nav,PhotoSlideshow,SponsorCarouselContainer}.tsx`,
`lib/queries.ts`, and new
`tests/contracts/academy-clear-frame-loading.test.ts`. The earlier design
preview files and images remain in this worktree. No database migration,
production write, push, deployment, or PR was made.

**Verification:** TypeScript and `git diff --check` passed. Focused existing
contracts 155/155, new Clear Frame contracts 3/3, full contracts 945/945,
architecture 21/21, local database 245/245, and full suite 1576/1576 passed.
`next build` passed with local Supabase test credentials; its existing Edge
`process.version` warning remains. The first DB attempt was blocked by sandbox
loopback `EPERM`; the same suite passed after local-only loopback access was
granted. In the built app at `alpha.localhost:3225`, the available academy
fixture rendered the crest, server Programs link, hero, match and story; no
error overlay or browser console error appeared, no skeleton remained after
network idle, and mobile width 390 had no horizontal overflow. The mobile
menu exposed its Programs entry. Afterward, the documented local Diverse City
fixture was restored with `scripts/import-diverse-city-local.ts`; its import
reported an idempotent replay and zero hosted mutations. The local server was
restarted with `ONZIO_ENVIRONMENT=staging` to match the fixture's primary
domain. `http://diverse-city.localhost:3225/` now returns 200, and the in-app
browser shows the actual crest, four Programs links, hero, kit, match, story,
and lower homepage content with no skeleton left after settling. Christian's
own visual review, slow-network transition, and reduced-motion acceptance
remain open. The academy Hero already uses the Diverse City video
asset for other academy fixtures; this loading change reuses its poster and
does not introduce another preview crest or media source.

**Exact next step:** Christian can review the running local preview at
`http://diverse-city.localhost:3225/`. Then inspect the actual Diverse City
first load on desktop and mobile (including slow network, Programs dropdown,
empty/failed data behavior, and reduced motion). Record
that evidence before considering a release. Keep the branch local until
Christian explicitly authorizes any push or deployment.

## Diverse City first-load exploration — 2026-09-24 (Codex)

Created and checked out `codex/diverse-city-loading-options` from the current
Homepage Editor branch head `561ff5c`; existing unrelated dirty files were
preserved. Another task switched the shared checkout afterward, so this branch
is now checked out in `/private/tmp/onzio-diverse-city-loading-options`.
Added `diverse-city-loading-options.html`, a standalone local preview
with four replayable approaches: poster-first, measured site skeleton, branded
intro, and progressive section loading. It uses the checked-in Diverse City
hero/story posters, the current academy palette, the published hero copy, and
three copied club images under `design-previews/diverse-city-loading/`.
No public application component, route, or test was changed.

The preview now combines each page treatment with a navbar treatment. It
models the academy's over-hero crest/affiliation header, desktop Programs
dropdown, mobile menu, and the four tenant program labels. Replay and the
slow-connection toggle drive navbar and page timing together; Open nav menu
shows how program links wait without shifting the main navigation.

Christian selected the **site skeleton** loading approach (#2). Added
`diverse-city-skeleton-styles.html`, a separate combined navbar/homepage
style study with four treatments of that same measured layout: static club
tint, soft sweep, poster veil, and linework. The study holds the skeleton for
comparison, can show the finished site or replay the transition, and includes
the desktop Programs dropdown and mobile menu. The first preview links to it.

Christian favored **Poster Veil** and requested three variations. Added
`diverse-city-poster-veil-variations.html` with the original as option 1 and
three variants: clearer photo, sky glass, and night film. All keep the same
navbar/page skeleton geometry and final-state content. The style study links
to this iteration. No public site implementation has started.

Read-only diagnosis: `HomePageClient` dynamically imports every homepage
section with `ssr: false`; `AcademyHomeShopFeature` and `AcademyNextMatch`
initially return `null` while fetching data; the academy hero mounts Bunny
video after hydration. `ClubBrandingProvider` starts with an empty tenant logo
and fetches branding after mount, so Nav first uses text initials; Nav also
starts with no Programs children and fetches them after mount. These can leave
gaps and cause the header and sections to change at different moments. The
production site was not visually measured in this pass.
The local `file://` preview was blocked by the app browser's URL policy, so a
visual browser pass is still open. Static HTML/JavaScript and asset-path checks
passed for the preview files; the optional CSS parser was unavailable because
this worktree has no installed `postcss`. Exact next step: let Christian choose
among the Poster Veil iterations, then implement the selected skeleton in the
shared `academy@1` presentation path with tenant-safe content and verify the
public homepage on desktop and mobile.
...
Agent: Claude Sonnet 5 (Claude Code), 2026-08-30. Status: **Done. `origin/staging` deleted (local + remote branch, plus its worktree removed) after verifying nothing was lost. Vercel's GitHub integration for this project is reconnected to `main` — a push to `main` now deploys to production again, same as any normal project.**

**Correction to every entry below dated 2026-08-29/30 that says production is "disconnected" or that "Git-triggered auto-deploy is now permanently disabled":** that was accurate at the time, as a deliberate temporary safety measure during incident response — not a permanent architectural decision. It has since been deliberately reversed, per Christian's explicit instruction, once `main` was fully reconciled, the redesign merged and reviewed, and every found bug fixed. From this point forward, **every push to `main` is a real production deploy** — treat it with the same rigor (verification + review) used for everything in today's entries, not as a routine commit.

**Why `staging` was retired, not just left alone.** Today's incident (see the entries below) happened because three things quietly diverged at once: `main`, `origin/staging`, and what was actually deployed to production — with no single branch anyone could trust as "what's really live." Contributing causes: hotfix branches were deployed straight to production via manual `vercel --prod` and never merged back into `staging` or `main`; and `staging` had accumulated work for an entirely unrelated client (Manu Ledesma Academy) mixed in with Diverse City FC/Columbus Lions FC work, so it never cleanly represented "what's about to ship" for any one tenant. The decision: drop the long-lived `staging` branch entirely. Going forward, new work happens on short-lived topic branches/worktrees (the pattern already in de facto use — `claude/cv-united-crest`, `claude/mla-pathway`, `claude/registration-forms`, `hotfix/*`) that merge into `main` and then get deleted, not a branch that lives for months. Once Vercel's Git integration is reconnected (see above), every branch/PR also gets its own Vercel preview deployment automatically — a real, hosted, per-change test environment that a shared `staging` branch never reliably was.

**What was verified before deleting `staging` (nothing was lost):**
- **MLA pathway work** — confirmed fully captured on `origin/claude/mla-pathway` (HEAD `5b3c08f`), which is an ancestor of `staging`'s history through that point. Untouched, still exists as its own branch.
- **The other ~20 non-MLA commits unique to `staging`** (registration-forms/Stripe Connect feature work, form archival lifecycle, minor/adult participant modes, programs/tryouts↔registration-form linking) — checked with `git cherry -v origin/main origin/staging` first (patch-equivalence), then verified for real by reading each commit's diff and cross-checking the actual current code/migrations on `main`, not just trusting commit messages or patch-id matches (which can false-negative when content reached `main` via a differently-shaped cherry-pick, as happened elsewhere today). Confirmed **all of it is already functionally present on `main`** — matching migration filenames (`20260821072452_registration_form_archiving.sql`, `20260820224337_registration_participant_modes.sql`, `20260821002508_link_registration_forms_to_programs_tryouts.sql`, etc.) and matching/superset code (`lib/registration-fields.ts`, `app/admin/(protected)/registrations/page.tsx`, `app/api/stripe/connect/route.ts`, and others). The one local-only "Special Kickers" native registration form draft was confirmed to have never gone live — the real diversecityfc.com Special Kickers program still uses an external Tally form — so nothing there was lost either.
- **One genuinely missing piece, preserved before deletion:** a reusable Stripe Connect go-live verification tool (`lib/stripe-connect-verification.ts`, `scripts/verify-stripe-connect-config.ts`, its contract test, and the `stripe:verify-connect-config` npm script) plus its companion checklist doc (`docs/stripe-live-go-live-checklist.md`). `main` had the equivalent tool for billing/portal but nothing for Connect. Cherry-picked onto `main` as two commits (`238774b`, `17aec22` — the second needed a `HANDOFF.md` conflict resolved by keeping `main`'s log and letting the cherry-pick's other file changes, including the new checklist doc, land alongside it).

**Verification of the two cherry-picks:** `npx tsc --noEmit` clean; `npm run lint` clean; `npm run build` clean; the new `tests/contracts/stripe-connect-verification.test.ts` passes 9/9.

**Working tree.** Two commits on `main` for the cherry-picks, pushed to `origin/main` before `staging` was deleted. `origin/staging` no longer exists (deleted both locally and on GitHub); the worktree that tracked it (`onzio-platform-diverse-city`) was removed. A stray uncommitted local change in that worktree (a partial revert of `app/admin/(protected)/seasons/page.tsx` back to an older, inferior shape) was confirmed worthless and discarded with the worktree — it was strictly behind what's already committed and live.

**Exact next step:** none pending from this entry. The repo now has one long-lived branch (`main`, reconciled with production and Git-connected again) plus whatever short-lived topic branches are actively in progress. Confirmed via direct checks that no other worktree on this machine has a stale local ref to `staging`, and that no instruction file (`AGENTS.md`/`CLAUDE.md`/`README.md`) tells a future session to push to or merge from it — the only remaining mentions of the word "staging" anywhere are either this file's own historical log entries (accurate records of the past, deliberately not rewritten) or unrelated concepts that still exist and were never in scope here (the separate Supabase staging *project*, and the media-upload staging *area* referenced in `AGENTS.md`'s media rules).
...
An 8-angle parallel code review of the redesign merge (`f4833e7..a96f7fe`, previous entry) confirmed nine real bugs; all nine are fixed here. **The grace-period/middleware lockout issue the review also raised remains OPEN — it is a product decision pending human input and was deliberately not touched (no changes to `middleware.ts` or any suspended-lockout behavior).**

1. **Unordered pagination could miscount paid registrations.** `readAllPaidRegistrations` (`lib/admin-dashboard-data.ts`) paged with `.range()` but no `.order()` — Postgres guarantees no stable row order across pages without one, so rows could be skipped/double-counted. Added `.order("id")`. Same bug class on `/admin/registrations`: its per-form counts used a single **unpaginated** `select("form_id,status")`, silently truncating at the PostgREST default 1000-row cap. Now pages 500 rows at a time ordered by `id` (same contract as the dashboard helper; direct reuse wasn't practical — that helper is paid-only on a server-side onzio-schema client, the page needs paid+total via the RLS-scoped admin client). This required adding `.range()` support to the admin data proxy: `lib/admin-client.ts` (builder), `lib/admin-data-contract.ts` (schema), `app/api/admin/data/route.ts` (handler).
2. **Two disagreeing "billing authorized" predicates.** The dashboard's Payments quick action gated on `isBillingAdminEmail` (the `BILLING_ADMIN_EMAIL(S)` env allowlist), while the sidebar/route-search/banner used `/api/stripe/billing-admin` → `requireBillingRouteAuthorization` (active owner, no allowlist) — so the two surfaces could disagree depending on an env var. The payments page itself redirects on `role !== "owner" || lifecycle === "archived"` with no allowlist, confirming the API-based rule is the intended one. `app/admin/(protected)/page.tsx` now uses `club.role === "owner" && club.lifecycle !== "archived"`; `tests/contracts/admin-dashboard.test.ts` updated to pin the corrected predicate (and the new pagination ordering). `lib/billing-admin.ts` itself is left in place but is now unreferenced by the admin portal.
3. **Programs drag-reorder: silent data loss + interleaving writes.** On a failed reorder, `persistProgramOrder`'s error path unconditionally called `loadPrograms()`, wiping any open unsaved editor draft. Now, if `dirty`, the reload is skipped and the error message says the shown order may not be saved; the draft survives. Also added an in-flight guard (ref + `reorderSaving` state) so a second drag can't fire while a reorder's per-row writes are still in flight (drag sensors disabled during the window, kept separate from the filter-based `canReorderPrograms` so the "clear the filter" hint doesn't flash).
4. **Footer preview lied about an empty tagline.** `/admin/branding`'s preview passed the raw tagline state; an empty field rendered NO tagline in the preview even though the live site maps stored-empty to `DEFAULT_ACADEMY_FOOTER_TAGLINE`. The preview override now runs through `resolveFooterTagline`, matching live behavior and the field's own "leave empty" copy.
5. **Footer preview showed Rose City sample social links as if live.** The preview override passed all `socialLinks` state, including `DEFAULT_SITE_SOCIAL_LINKS`' Rose City URLs for untouched platforms — links the real public footer never shows. Now filtered to `configuredSocialIds`, the exact same gate the save path uses.
6. **Escape closed both the ⌘K search AND an open side panel**, dropping the panel's unsaved form state. `AdminRouteSearch`'s Escape handler now stops the event — note `stopPropagation` alone was NOT enough (live-verified still broken with it): React delegates events on `document` in the App Router, the same node `AdminSidePanel` listens on, and stopPropagation never stops sibling listeners on the same node — so it also calls `event.nativeEvent.stopImmediatePropagation()`.
7. **Dashboard "today" computed in UTC.** `lib/admin-dashboard-data.ts` used `toISOString().slice(0, 10)`, dropping tonight's fixtures/tryouts off "Upcoming Fixtures & Events" hours before local midnight for US clubs. Now built from local `getFullYear`/`getMonth`/`getDate` (the same convention as `formatDateInput` in the schedule page).
8. **Internal dev notes rendering as admin-facing UI copy.** The schedule page's sponsor block rendered "This whole block is hidden for academy@1 and editorial@1…" and the contact page rendered "…every other template edits this page blind." as visible paragraphs. Both moved into real code comments; the rendered paragraphs removed (the surrounding UI already carries the genuinely admin-facing copy).
9. **Stale comment on `/admin/members`.** A comment claimed "whoever is viewing this page is always the club owner"; in reality the page is a client component with no role redirect — a non-owner admin can load it by URL and simply gets empty/error chrome because the API correctly rejects them (server-side gating is intact; this was never a security bug). Comment corrected to describe reality; no behavioral guard added (deliberately out of scope).

**Verification:** `npx tsc --noEmit` clean; `npm run lint` clean (no warnings); `npm run build` clean; full suite with `.env.test` exported: **1387/1387 passing**, including the updated `admin-dashboard` contract test. Live browser pass on the local Alpha stack (`onzio-platform-alpha-preview`, port 3008, `owner-aal2@alpha.local` OTP login): Branding footer preview keeps the standard tagline with the field cleared (fix 4) and renders only the club's one genuinely-configured platform (Instagram) while all five form fields display Rose City sample URLs (fix 5); with a roster "New Player" panel open and text typed, ⌘K + Escape closes only the search — the panel and its unsaved text survive (fix 6, re-verified after the stopImmediatePropagation correction); the dashboard now shows the Payments quick action for the Alpha owner with no `BILLING_ADMIN_EMAIL` set, matching the sidebar (fix 2).

**Open for a human:** the grace-period/middleware lockout behavior (whether/how suspended clubs are locked out of the admin portal) is still an unresolved product decision — nothing in this pass changed it.
...
**What happened.** A separate, ongoing effort (documented in the block of entries directly below this one, and the "Current State" section further down this file) had been incrementally porting features from a `staging` snapshot into `main`, phase by phase, on the assumption that `staging` represented the furthest-ahead, most-complete version of the codebase. That assumption was correct when the promotion effort began, but stopped being correct at some point after: production (`diversecityfc.com`) had continued receiving direct `vercel --prod` hotfix deploys — including the full programs/contact/tryouts/registrations pages, video/Bunny.net content, and the presentation-template system — that were never merged back into `staging`, `main`, or any branch in this repository. `origin/main` and what was actually running in production had diverged by 496 files.

Neither branch's history knew about the other's divergence. When the promotion effort's PR #4 (`1d2dd1c`, "Phase 3: members route") was merged to `main` and `main` was connected to Vercel's Git integration for auto-deploy, the push silently overwrote production with `main`'s older, less-complete snapshot — downgrading the live site. This was caught, and PR #4's merge was reverted on `main` (`bc565e8`) as an immediate stop-gap.

**The fix, in two parts:**
1. **Production restored.** The real, working code (`origin/hotfix/registration-live-mode-prod` at `6176cd9`) was redeployed directly to production via `vercel --prod`, bypassing Git entirely.
2. **Hard safety guarantee.** Vercel's GitHub integration for this project was fully disconnected in the Vercel dashboard. Nothing pushed to any branch in this repository — including this reconciliation — can trigger a production deploy going forward. Any future production deploy is a deliberate, manual `vercel --prod` action, gated by this file's own pre-existing "Mandatory gate before any production Vercel deploy" migration-parity check.

**This reconciliation.** With production safe and disconnected, `main` (`bc565e8`) was merged with `origin/hotfix/registration-live-mode-prod` (`6176cd9`) via `git merge --no-ff` (not a tree-reset) so both branches' full commit history is preserved. Default conflict rule: hotfix's version wins, since it is the real, already-live, working production code and `main`'s competing version was almost always the older/redundant reimplementation from the promotion effort. Two deliberate exceptions:

- **`components/ui/popover.tsx`** — hand-merged. `main` had independently fixed a real bug: Base UI's `Popover.Portal` defaults to rendering into `document.body`, a sibling of the `.admin-theme` wrapper div, not a descendant — so portaled popover content lost every themed CSS variable. `main`'s fix defaults the portal container to the nearest `.admin-theme` ancestor via a `useDefaultPopoverPortalContainer` hook, falling back to `document.body` when none is found. Hotfix's independently-evolved `popover.tsx` never had this fix. The final file keeps hotfix's current structure/exports and adds `main`'s hook and the `container={defaultContainer}` prop on top.
  - **Caveat worth flagging:** the `.admin-theme` wrapper itself comes from a `main`-only admin light/dark theme toggle feature (`components/admin/AdminThemeProvider.tsx`, `AdminThemeToggle.tsx`, `lib/admin-theme.ts`) that hotfix never adopted at all — hotfix's `AdminShell.tsx`/admin layout hardcode a dark-only shell and, per the default conflict rule, hotfix's versions of those two files were kept (they are also more complete/current in unrelated ways — richer grouped nav with an owner-only "Team access" entry, `isEditorialTemplate` filtering, and a `requireFreshClubSession` security check `main`'s layout lacked entirely). The three admin-theme files still exist in the tree post-merge (they merged in cleanly, no conflict) but are now unreferenced by anything — the toggle button is not rendered and nothing wraps children in `.admin-theme`. The ported popover fix is therefore currently a no-op (its `document.querySelector(".admin-theme")` call will simply find nothing and fall back to `document.body`, hotfix's original behavior) rather than a regression. Whether to wire the toggle feature in properly or delete the orphaned files is a follow-up decision, not made here.
- **`.github/workflows/roster-media-smoke.yml`** and **`tests/README.md`** — took hotfix's content, but preserved `main`'s own later fix (commit `10559e5`, "Use the new Rose City Vercel domain") on the one line it touched in each file, so the roster media smoke check still points at `https://onzio-rcfc.vercel.app` rather than reverting to the older `https://onzio-platform.vercel.app` placeholder.

The members ("Team access") feature and the media-cleanup cron feature both auto-merged cleanly with no conflicts — in both cases hotfix's files (`lib/owner-admin-membership.ts`, `lib/operator/invite-club-member.ts`, `lib/operator/manage-membership.ts`, `app/admin/(protected)/members/page.tsx`, `app/api/cron/media-cleanup/route.ts`, `lib/media-cleanup.ts`) landed byte-identical to their pre-merge content, and `main` had no competing files under either feature to delete — `main`'s only members-related file (`lib/operator/manage-membership.ts`) was already byte-identical to hotfix's version at the shared path. `app/admin/(protected)/seasons/page.tsx` was checked directly and found byte-identical between `main` and hotfix — no porting needed.

All other conflicts (`lib/media-processing.ts`, `package.json`/`package-lock.json`, `styles/globals.css`, `tailwind.config.ts`, `app/admin/(protected)/layout.tsx`, `components/AdminShell.tsx`, `tests/architecture/platform-architecture.test.ts`, `tests/contracts/admin-mobile-navigation.test.ts`, `vercel.json`) were resolved by taking hotfix's version entirely, per the default rule. `package.json`'s `lucide-react`/`motion` versions and the `tailwindcss-animate` dependency reverted to hotfix's slightly older pins; the three orphaned admin-theme files only use `lucide-react`'s `Moon`/`Sun` icons (present in both version ranges), so this did not break their compilation.

**Verification:** see the entry immediately following this one (or the commit this entry ships with) for `tsc`/`lint`/`build`/test results.

**Working tree.** Single merge commit on `main`, fast-forwarded and pushed from `bc565e8`. No production database, Stripe, or hosted state touched — production itself was already fixed and disconnected from this repo before this merge began.

**Exact next step:** `origin/staging` still carries 24 commits of its own unmerged work (the original admin-portal-redesign, MLA pathway, registration-forms/Stripe Connect) — a separate, explicitly out-of-scope reconciliation for another day. The orphaned admin-theme toggle feature noted above needs a human decision (wire it up or remove it). Any future production deploy must go through a deliberate manual `vercel --prod` per this file's existing migration-parity gate, since Git-triggered auto-deploy is now permanently disabled for this project.
...
## Consolidated log: `staging` → `main` promotion effort (superseded by the reconciliation entry at the top of this file)

The entries below are preserved as-is from `main`'s own HANDOFF.md history prior to the reconciliation merge described at the top of this file. They document the incremental, phase-by-phase effort to port a `staging` snapshot into `main`, which was unaware that production had already diverged further ahead via direct `vercel --prod` hotfix deploys that were never merged back into any branch. This effort — and everything it ported — is superseded by the reconciliation merge; `main` now matches what was actually live in production, plus the specific pieces salvaged from `main` as described above. Kept here for the historical record, not as a guide to `main`'s current state.
