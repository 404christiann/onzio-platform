...
**Note on this HANDOFF file's own gap.** Between the entry below (recorded when the redesign was "ready to commit") and this one, a full pre-merge code review (8 findings, including a critical cross-tenant social-links leak, all fixed and independently re-verified), a second commit (`c550c7e`, the fixes), a PR (#1, against `staging`, not `main` — `staging` and `main` had already diverged too far for a `main`-targeted PR to be sane), and the actual merge into `staging` (merge commit `3f9f24c`) all happened — but no HANDOFF entry was written for any of that before this one, because the commit/PR/merge flow moved directly from verification to git operations without a documentation step in between. Recording it here for the record rather than leaving it silently missing: `staging` now contains the full admin-portal-redesign work (both commits), live and merged, as of `3f9f24c`.

**Why the migration work came up.** After that merge, Christian asked how to get the work into `main` (production). Investigation found `staging` and `main` have diverged by 259 commits / 628 files / ~138K lines — not a simple staleness gap. A live test cherry-pick of the redesign's 2 commits onto `origin/main` (tested locally, fully aborted/cleaned up afterward, nothing pushed) produced 34 conflicts (21 content, 13 files missing from `main` entirely), including `components/ui/popover.tsx` (a shared primitive the redesign depends on) and most of the individually-redesigned admin page files.

**What the investigation found (3 parallel research agents, all read-only, no code changed):**
- The 13 files missing from `main` trace to 7 coherent, sequential epics on `staging` spanning 2026-08-02 to 2026-08-20 (Diverse City Phase 5 release → PLAT-101 passwordless admin auth → shadcn/ui admin restyle → PLAT-104 sidebar nav unification → Lions editorial shell → editorial polish → Stripe Connect registration forms), with overlapping file ownership — meaning they'd need promoting together and in order, not individually.
- `main`'s admin pages are largely the **same product at an earlier point** (same multi-tenant `useClubContext`/`club_id` foundation, same core types/functions) for most files — genuinely mergeable, not a rewrite.
- **`components/AdminShell.tsx` is a real exception**: `main`'s version is hardcoded to a single tenant (literal `"Rose City FC"` alt text, hardcoded colors, no `useClubContext`), while `staging`'s is a full multi-tenant rebuild on a new shadcn/Radix-style component primitive system. This is the piece the redesign most directly depends on, and it isn't a narrow cherry-pick target.
- Framework/dependency versions match exactly between the branches (Next 15.5.22, React 19.2.4, same Supabase/Tailwind majors) — no version-fork risk there.
- `staging` has 30 Supabase migration **files** that don't exist in `main`'s git history — but critically, checking the *actual production database* (`supabase migration list --linked` against project `ioalthwsdrlzrubomrow`) showed only **one** of those was genuinely unapplied to the live database; the rest had already been pushed to production directly at various points, independent of `main`'s git state ever catching up. This repo's own history contains a commit titled "Add mandatory migration-parity gate after a production incident" — i.e. this exact class of drift caused a real outage before, which is why the gate exists and why this was checked live rather than inferred from git alone.

**The one migration applied.** `20260815140402_pathway_presentation_template.sql` — widens `onzio.presentation_documents`'s `template_id` check constraint to add `'pathway'` to the allowed set (for the unrelated MLA pathway@1 project). Verified before applying: purely additive (no data mutation, no drop), and chains cleanly from the exact constraint state the last-applied migration touching that table (`20260812120000_editorial_presentation_template.sql`) left in place, with nothing in between touching the same table. Applied via `supabase db push --linked --include-all` (run by Christian directly, in his own terminal, with the database password — not entered through this agent, per this session's credential-handling boundary). Re-verified after: `supabase migration list --linked` now reports 40 migrations, 40 matches, 0 drift.

**Decision: full `staging` → `main` promotion is explicitly deferred, not attempted today.** Given the `AdminShell.tsx` architecture gap and the 7-epic dependency chain found above, this reads as a multi-day reconciliation effort, not something to rush alongside shipping today's redesign work. Christian agreed to defer it as its own planning effort rather than force it today.

**Working tree.** No code files were changed in this repo for this entry — only this HANDOFF record and the live production database. `git log` shows no new commits from this entry; `main` and `staging` remain exactly as diverged as described above, aside from now having identical applied-migration state on the shared production database.

**Exact next step:** the `staging` → `main` promotion itself, whenever Christian wants to take it on — likely starting with a deliberate reconciliation of `AdminShell.tsx`'s multi-tenant rebuild (the piece everything else depends on), then working through the 7 identified epics in their dependency order, re-running this same live-migration-parity check (now a fast, cheap, known-good check) before any future production code deploy. Still open: whether Dashboard/Payments should get the skeleton-tier loading treatment, and confirming the redesign looks right live on `staging` now that it's actually merged there.
